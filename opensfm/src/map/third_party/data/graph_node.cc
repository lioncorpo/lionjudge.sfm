...
void graph_node::update_covisibility_orders() {
    ...
    std::vector<std::pair<unsigned int, map::Shot*>> weight_shot_pairs;
    ...
    // sort with weights
    std::sort(weight_shot_pairs.rbegin(), weight_shot_pairs.rend(), [](const auto &x,const auto &y) { return x.first < y.first && x.second->id_ < y.second->id_; } );
